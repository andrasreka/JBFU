...
def apply_bilateral_filter(f, g, x, y, diameter, sigma_i, sigma_s):
    ...
    for i in range(diameter - 1):
        for j in range(diameter - 1):
            ...
            gi = gaussian(mean_i, sigma_i)
            gs = gaussian(mean_s, sigma_s)
...
def jbf(f, g, filter_diameter, sigma_i, sigma_s):
    ...
    filtered_image =  Parallel(n_jobs=20)(delayed(apply_bilateral_filter)(f, g, i, j, filter_diameter, sigma_i, sigma_s) for i in range (len(g)) for j in range (len(g[0])))
    return np.array(filtered_image, dtype = np.uint8).reshape(g.shape)

def upsampling(rgb, depth):
